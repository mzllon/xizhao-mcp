...
**目标：** 交付 XM-SQL-MCP v1 Client 模式——本地 Stdio MCP 服务代理 AI 访问 MySQL，提供 AST 策略引擎、tamper-evident 审计日志、self-approval 工作流、本地 Dashboard。

**架构：** v1 以 `core/` 为真相来源，CLI、MCP tools、Web API 都只是传输层包装。MCP 通道走 Stdio，无 HTTP 认证；Dashboard 作为独立命令按需启动，使用本地 HTTP + Jupyter 风格 token 认证，与 `xm-sql-mcp client` 进程并存。
...
- CLI 命令：`xm-sql-mcp setup` / `client` / `dashboard` / `conn <subcmd>` / `policy <subcmd>` / `audit [filters]`
- 连接配置加密存储，默认目录 `~/.xm-sql-mcp/`
...
- **永不写 stdout**（MCP 协议占用），所有日志走 stderr + `~/.xm-sql-mcp/logs/xm-sql-mcp.log`
...
│   │       ├── client.ts
│   │       ├── dashboard.ts
│   │       ├── conn.ts            # 新增: xm-sql-mcp conn <list|add|edit|delete|test>
│   │       ├── policy.ts          # 新增: xm-sql-mcp policy <show|set>
