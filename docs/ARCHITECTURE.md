...
v1 有两个独立进程，共享同一套文件系统（`~/.xm-sql-mcp/`）：
...
│                                                                     │
│  ┌──────────────┐  stdin/stdout   ┌───────────────────────────┐     │
│  │  MCP 客户端   │ ◄─────────────► │  xm-sql-mcp client (进程 A)   │     │
...
| 进程 B 随用随启                        | `xm-sql-mcp dashboard` 不常驻，审批/配置时才启动             |
...
```
MCP 客户端                    xm-sql-mcp client 进程内部                    存储 / MySQL
─────────                    ──────────────────────                    ───────────
...
xm-sql-mcp client
  ...
  ├─ 日志 (pino)
  │   ├─ 输出到 stderr
  │   └─ 输出到 ~/.xm-sql-mcp/logs/xm-sql-mcp.log
  └─ 优雅关闭
      ├─ SIGINT / SIGTERM 触发
      ├─ 拒绝新请求
      ├─ 等待 in-flight 最长 5 秒
...
### 3.2 `xm-sql-mcp dashboard`（按需进程）

```
xm-sql-mcp dashboard [--port 9020]
  ├─ Hono HTTP Server
  ...
  └─ 启动时
      ├─ 生成 32 字节 token
      ├─ 写入 ~/.xm-sql-mcp/dashboard.token (mode 600)
...
```
xm-sql-mcp client                    xm-sql-mcp dashboard
  │                                    │
...
XM-SQL-MCP **不在目标 MySQL 上创建任何表或对象**。它只是：
...
└─────────────────────────────────────────────────┘
        │                    │                │
        ▼                    ▼                ▼
  ~/.xm-sql-mcp/            ~/.xm-sql-mcp/      目标 MySQL
...
XM-SQL-MCP 的核心定位差异：**不是"给 AI 一个 MySQL 终端"，而是"在 AI 和 MySQL 之间插入一个可审计的安全代理"**。
