...
# 路径 B：常用者全局安装
npm install -g xizhao
xizhao setup
```

README 主推 A 路径（"30 秒上手"），文档中说明 B 路径。

**配置命令**：`xizhao setup` 是单一入口的交互式向导，串行 7 步：
...
跨平台路径解析（Windows 上是 `%USERPROFILE%\.xizhao\`）使用 [`env-paths`](https://www.npmjs.com/package/env-paths)。
...
2. 根据探测结果输出对应格式（如 `~/.codex/config.toml` 的 `[mcp_servers.xizhao]` 段，或新版可能的 JSON 格式）。
...
```
xizhao setup           # 首次配置（交互式向导）
xizhao client          # MCP Stdio 服务（被 AI 客户端调用，非交互式）
xizhao dashboard       # 启动本地 Web 控制台（按需）
xizhao conn list|add|edit|delete|test
xizhao policy show|set
xizhao audit [--since] [--deny-only] [--sql <pattern>]
xizhao --version
xizhao --help
```

- `xizhao client` 必须严格非交互（stdin/stdout 不能被 prompt 打扰）。
- 其他命令可交互。
...
### 本地 Web 控制台：`xizhao dashboard`

**v1 同时实现**，与 Server 模式（v2）共用前端代码。

- 按需启动：`xizhao dashboard` → 默认 `localhost:9020` → 自动开浏览器。
- 可与 `xizhao client` 同时运行，**两个命令独立但共享同一份 Self Storage**。self-approval 工作流依赖用户在 MCP 会话中打开 Dashboard 完成审批。
...
- 同事要等 v2（Server 模式）才能共用 Xizhao。v1 期间他们要么用竞品，要么暂时裸连。
