...
启动 `xm-sql-mcp dashboard` 时：

1. 生成 32 字节随机 token
2. 写入 `~/.xm-sql-mcp/dashboard.token`（mode 600）
...
CSV 导出（PRD A-04）v1 不做，需要时用 CLI `xm-sql-mcp audit --export csv`。
...
- Dashboard 默认 `9020`，可通过 `--port` 参数或 `~/.xm-sql-mcp/config.db` 中覆盖。
