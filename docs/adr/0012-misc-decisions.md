...
| 应用日志   | `~/.xm-sql-mcp/logs/xm-sql-mcp.log` + stderr                    | pino 结构化 JSON     |
...
**SQL 全文默认入日志**（debug 排错需要），但提供 `XM_SQL_MCP_LOG_SQL=off` 环境变量关闭（仅记 SQL hash）。
...
v1 不做 `xm-sql-mcp export/import`。手动备份 `~/.xm-sql-mcp/` 目录即可（含 master.key + config.db）。

### 更新机制（不做自动）

v1 手动更新。npm 用户 `npm update -g xm-sql-mcp`，二进制用户重新下载 GitHub Releases。**不做**自动更新检查（隐私优先、纯本地工具哲学）。

### 发布渠道

- **npm**（主渠道）：`npm install -g xm-sql-mcp` 或 `npx xm-sql-mcp`
...
v1 不提供 `xm-sql-mcp demo` 命令、不预置样本数据库。

**理由**：

- XM的价值在"AI 安全访问真实数据库"，demo 库失去真实性
- 用户都有 MySQL（dev/test 库场景），无需 XM-SQL-MCP 提供
...
- Windows：`%USERPROFILE%\.xm-sql-mcp\`
- macOS / Linux：`~/.xm-sql-mcp/`
...
- 不做配置导入导出，用户换机器时需要手动复制整个 `~/.xm-sql-mcp/` 目录。
