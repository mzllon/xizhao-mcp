...
在设计 MCP 工具的返回格式时，遇到一个反复出现的取舍：当数据库本身能给出某个信息的权威表述（如 `SHOW CREATE TABLE`、`EXPLAIN FORMAT=JSON`），XM-SQL-MCP 应该 (a) 直接转发，还是 (b) 解析后用自定义结构化字段重新表达？
...
**XM-SQL-MCP 的角色是策略与审计，不是数据库语义翻译。**
...
3. **零维护成本**：MySQL 升级新特性（如 MySQL 9 的 VECTOR 类型）时，XM-SQL-MCP 不需要改任何代码。
...
- 跨数据库迁移时，AI 要适应不同方言的 DDL/EXPLAIN，而不是享受 XM-SQL-MCP 提供的"统一抽象"。这是有意为之——抽象层会损失信息。

**反例（不适用本原则的场景）**：

- `list_tables`：返回 `{ name, type }[]` 而不是 `SHOW TABLES` 的纯文本。理由：`SHOW TABLES` 输出格式简陋（无 type 列），不是"权威表述"，只是简化的展示。XM-SQL-MCP 这里的组合（合并 `information_schema.tables`）是**增值**而非**重写**。
